...
public:
  ...
  /**
   * Saves the vocabulary from a binary file
   * @param filename
   */
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::loadFromBinary(const std::string &filename)
{
	...
	Meta m;
	memcpy(reinterpret_cast<char*>(&m), buf, sizeof(Meta));
	m_k = m.m_k;
	m_L = m.m_l;
	int n1, n2;
	n1 = m.m_sc;
	...
	int nodeCount = (size - sizeof(Meta)) / sizeof(NodeBin) - 1;

	for(int i = 0; i < nodeCount; i++)
	{
		NodeBin ndb;
		memcpy(reinterpret_cast<char*>(&ndb), buf + sizeof(Meta) + sizeof(NodeBin) * i, sizeof(NodeBin));
		...
		bool nIsLeaf = ndb.isLeaf;

		m_nodes[nid].descriptor.create(1, 32, CV_8U);
		unsigned char *p = m_nodes[nid].descriptor.ptr();
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::saveToBinary(const std::string &filename) const
{
	std::ofstream f;
	...
	n1 = (int)m_scoring;
	n2 = (int)m_weighting;
	Meta m = {m_k, m_L, n1, n2};
	char* buf = new char[sizeof(Meta)];
	memcpy(buf, reinterpret_cast<const char*>(&m), sizeof(Meta));
	f.write(buf, sizeof(Meta));
	delete[] buf;

	int size = sizeof(NodeBin) * (m_nodes.size() - 1);
	...
	for(size_t i=1; i<m_nodes.size();i++)
	{
		const Node& node = m_nodes[i];
		NodeBin nb;
		nb.parent = node.parent;
		nb.weight = node.weight;
		memcpy(nb.descriptor, node.descriptor.ptr(), sizeof(unsigned char) * 32);
		if(node.isLeaf())
			nb.isLeaf = true;
		else
			nb.isLeaf = false;

		memcpy(buf2 + sizeof(NodeBin) * (i-1) , reinterpret_cast<const char*>(&nb), sizeof(NodeBin));
		//f << F::toString(node.descriptor) << " " << (double)node.weight << endl;
	}
	f.write(buf2, size);
	delete[] buf2;
