	...
	void OnTouch(Dali::Actor actor, const Dali::TouchData &touch) override
	{
		if (touch.GetPointCount() > 0)
		{
			...
			if (touch.GetState(0) == PointState::DOWN)
			{
				...
				apple->ApplyForce( worldToPlaneVec(stw.direction) * 4.0, wVector3(0, 0, 0));
			}
		}
	}
...
private:
	...
	void PlaceObjects()
	{
		...
		for(int i = 0; i < objs.size(); i++)
		{
			...
			FrameActor* actor;
			if(modelName == "terrain-plane-plain" || modelName == "block_wood1_LOD0" || modelName == "block_wood2_LOD0" )
			{
