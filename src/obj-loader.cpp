  ...
  while ( std::getline( ss, line ) )
  {
    ...
    else if ( tag == "f" )
    {
      ...
      if( numIndices == 3 )
      {
        for( int i = 0 ; i < 3; i++ )
        {
          triangle.pointIndex[2-i] = ptIdx[i] - 1 - pntAcum;
          triangle.normalIndex[2-i] = nrmIdx[i] - 1 - nrmAcum;
          triangle.textureIndex[2-i] = texIdx[i] - 1 - texAcum;
        }
      ...
      else if( numIndices == 4 )
      {
        ...
        for( int i = 0 ; i < 3; i++ )
        {
          ...
          triangle2.pointIndex[2-i] = ptIdx[idx] - 1 - pntAcum;
          triangle2.normalIndex[2-i] = nrmIdx[idx] - 1 - nrmAcum;
          triangle2.textureIndex[2-i] = texIdx[idx] - 1 - texAcum;
        }
